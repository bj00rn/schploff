...
def set_photo(url, filename, paramList, outDir):
  ...
  try:
    ...
    oldImage = Image.new(3, (1280, 960), new_background = (255, 255, 255, 255))  
  ...
  draw.rectangle(((0, height-(fontsize+fontpadding)*len(paramList)), (width, height)), fill="black")
  idx=0
  ...
  for item in paramList:
    ...
    draw.text((0, height-idx*(fontsize+fontpadding)),
      "{value} {unit} ({station} {updated} {parameter})".format(
        ...
        unit=item['unit'],
        updated="{:%F %H:%M:%S}".format(datetime.fromtimestamp(item['updated']/1000)),
...
def main(argv):
  ...
  paramList = [ 
    fetchParameter(station=87440,parameter=3, source='metobs', period='latest-hour'),
    fetchParameter(station=87440,parameter=4, source='metobs', period='latest-hour'),
    fetchParameter(station=33008, parameter=1, source='ocobs', period='latest-day'), # Vaghöjd, signifikant 30 mi
    fetchParameter(station=33008, parameter=9, source='ocobs', period='latest-day'), # Vagperiod timvärde
    fetchParameter(station=33008, parameter=8, source='ocobs', period='latest-day'), # Vagriktning vid Tp (energimax 30 min)
    fetchParameter(station=33002, parameter=1, source='ocobs', period='latest-day'), # Vaghöjd, signifikant 30 mi
    fetchParameter(station=33002, parameter=9, source='ocobs', period='latest-day'), # Vagperiod timvärde
    fetchParameter(station=33002, parameter=7, source='ocobs', period='latest-day'), # Vagriktning timvärde
  ]
  ...
  set_photo(url="http://83.140.123.183/ImageHarvester/Images/3009-landsort_1_1280.jpg", filename="landsort_{0}.jpg".format(datestring), paramList=paramList, outDir=out_dir)
